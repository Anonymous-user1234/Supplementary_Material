...
This provides supplementary materials for an **anonymous submission under review**, including the experimental code and reference circuit implementations used in the paper.


## Latexdiff file.

As requested by the submission system, we provide a latexdiff version comparing the previous submission and the current revised manuscript.

Please note that, after the major revision, the manuscript was substantially reorganized and partially rewritten to improve clarity and presentation. As a result, the structural changes are extensive, and the latexdiff file may not fully reflect the logical correspondence between the two versions. 

We therefore recommend referring primarily to the revised manuscript for a coherent presentation of the updated work.

## Requirements 
(Tested on a multi-core system with 32 CPU cores and 128 GB of RAM, running Ubuntu 20.04 LTS.)
...
* **TARGET_STRATEGY** (Sec. 3.5):

  * `1` AT (Unweighted startegy)
  * `2` ST (Weighted startegy)
